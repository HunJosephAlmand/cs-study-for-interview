...
* 수평 파티셔닝과 비교
  ...
  * 샤딩 데이터베이스 수준의 트랜잭션이 불가능하다. => 애플레케이션 계층에서 트랜잭션을 구현해야함
  * 데이터베이스 스키마 그대로 유지
* 애플리케이션의 규칙에 따라 샤딩
* 방식
  * 알고리즘 샤딩
    * 각 레코드에 대해 샤딩 키 값을 생성하여 키 값에 해당하는 노드에 저장
    * 페이로드 크기나 공간 활용도 고려 X
    * 쿼리에 파티션 키가 반드시 있어야 함
...
* master(primary) 데이터베이스
  * 데이터에 대한 수정/삭제/삽입 요청 제어
  * 요청에 따른 변경 내용은 비동기적으로 slave 데이터베이스들에게 전파
