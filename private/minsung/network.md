...
* FIN 패킷을 받은 서버는 연결을 닫을 준비(CLOSE_WAIT) 상태가 되고, FIN 패킷을 성공적으로 받았다고 클라이언트에게 ACK 패킷을 전송
* 데이터를 전부 전송한 서버는 클라이언트에게 FIN 패킷을 전송
...
* 송신한 패킷에 대한 ack 응답이 올 때 까지의 시간을 통해 네트워크 상황을 분석한다.
* slow-start, 
...
* fast retransmit
  * 타이머를 이용해 보낸 패킷이 손실되었는지 파악한다.
  * 같은 acknowledgment를 3번 이상 받을 경우, 순서대로 보낸 패킷이 중간에 손실되었다는 뜻이므로, 타이머를 기다리지 않고 해당 패킷부터 다시 재전송한다.
* fast recovery
...
* acknowledgment로 전송에 실패한 패킷을 재전송할 수 있다.
