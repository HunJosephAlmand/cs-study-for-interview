...
- 3way-handshake, 4way-handshake
  - 특정 시간 끝나면 클라이언
- 흐름제어, 혼잡제어, 오류제어
  - 윈도우 크기는 3way handshake 과정에서 설정됨
...
- 연결형 통신과 비연결형 통신의 차이?
  - 연결형 통신은 가상 회선 구축
    - "연결이 되었음" -> 신뢰성 있는 통신
    - TCP의 경우 패킷 데이터를 테이블에 보관하는 식으로 돌아감
    - 세션
  - 비연결형 통신은 수신만 함



- 4way hanshake에서 대기시간이 존재하는 이유?



- 네트워크 공격
  - 세션 하이재킹
    - TCP연결을 클라이언트 대신에 물려받는 방식



- 난수 시퀀스 넘버를 처음에 보내는 이유?
  - 보안 상의 이유
...
- IPv4
  - 32비트 값, 8비트의 4부분으로 나뉨
  - 각 8비트 값을 10진수로 표현한 숫자를 `.`으로 구분(0 ~ $2^8 - 1$(255))
  - 최대 $2^{32}$개(약 43억 개)의 고유 주소 부여 가능
  - Class-B
  - 사이더
  - 할당 중지 선언(2011)
  - 주소 고갈(2015)
...
- IPv6
  - 128비트값
  - (4자리 16진수 숫자) * 8개로 표현, `:`으로 구분
  - `0`이 연속되는 경우 `::`로 생략 가능
  - 보안 프로토콜 IpSec
- 통신방식도 변화함
  - UniCast 1:1통신
  - BroadCast 네트워크 내 모든 장치에 뿌림
    - IPv4
    - 공유기에서 사용하는 방식
  - MultiCast 연결되어 있는 모든 노드
    - 여러 상대에게 전달
  - AnyCast 가장 가까운 노드에 전송
    - IPv6