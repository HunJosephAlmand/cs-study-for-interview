...
>>>>>>> 3c3c300bf12a314d533b09945ca344d9d36176fb
...
=======
- Context switch는  이전 프로세스가 사용하던 Cache 메모리를 지운다.
>>>>>>> 3c3c300bf12a314d533b09945ca344d9d36176fb
...
<<<<<<< HEAD
- 하나의 서버 요청 스레드가 blocked 상태에서도 동일한 태스크 내의 다른 스레드가 실행 되어 빠른 처리를 할 수 있다.
=======
...
- 라이브러리 형태(User Thread)
  ...
  - 사용자 프로그램이 스스로 스레드를 관리
    - 운영체제가 스케줄링 하듯이 프로세스가 CPU 제어
<<<<<<< HEAD
  - 해당 프로세스는 가상 머신처럼 구동
=======
  - 가상 머신에 의해 구현된다.
...
<<<<<<< HEAD
    * 실제 가상 머신 프로그램에서는 non-blocking I/O를 통해 해결한다고 함.
=======
    * 실제 가상 머신에서는 non-blocking I/O를 통해 해결한다고 함.
>>>>>>> 3c3c300bf12a314d533b09945ca344d9d36176fb
      * 호출한 thread만 프로세스 내부적으로 block시키고 non-blocking I/O를 호출한 다음, 다른 스레드들을 실행
      * non-blocking I/O만 호출하는 방식으로 코드를 짜서 해결하는 경우도 있음
